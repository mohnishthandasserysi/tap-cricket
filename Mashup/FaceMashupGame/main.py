...
import os
import random
from utils import MashupGenerator, FaceProcessor


class FaceMashupGame:
    def __init__(self):
        """Initialize the Face Mashup Game."""
        self.mashup_generator = MashupGenerator()
        self.score = 0
        self.rounds_played = 0
        self.window_name = "Face Mashup Game"
    ...
    def create_game_display(self, mashup_image: np.ndarray, options: list, feedback: str = "") -> np.ndarray:
        ...
        # Add title
        title = "Face Mashup Game - Guess the Players!"
        cv2.putText(canvas, title, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    ...
    def start_new_round(self):
        ...
        try:
            ...
            self.current_mashup, used_players = self.mashup_generator.create_mashup()
    ...
    def run(self):
        ...
        print("🎮 Starting Face Mashup Game!")
        ...
        try:
            ...
            if len(players) < 3:
                ...
                print("📝 Add images named like: player1.jpg, player2.jpg, etc.")
                return
            
            print(f"✅ Found {len(players)} players: {', '.join([name for name, _ in players])}")
...
def main():
    """Main function to run the Face Mashup Game."""
    print("🎭 Face Mashup Game")
    print("=" * 30)
    ...
    try:
        ...
        print("- Required packages installed (run: pip install -r requirements.txt)")
        print("- Internet connection for downloading dlib model (first run only)")
