...
import os
import random
from simple_utils import SimpleMashupGenerator


class SimpleFaceMashupGame:
    def __init__(self):
        """Initialize the Simple Face Mashup Game."""
        self.mashup_generator = SimpleMashupGenerator()
        self.score = 0
        self.rounds_played = 0
        self.window_name = "Simple Face Mashup Game"
    ...
    def create_game_display(self, mashup_image: np.ndarray, options: list, feedback: str = "") -> np.ndarray:
        ...
        # Add title
        title = "Simple Face Mashup Game - Guess the Players!"
        cv2.putText(canvas, title, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        # Add subtitle
        subtitle = "(Using OpenCV Face Detection)"
    ...
    def run(self):
        ...
        print("🎮 Starting Simple Face Mashup Game!")
        print("📁 Make sure you have player images in the 'players/' folder")
        print("💡 This version uses OpenCV face detection (no dlib required)")
...
def main():
    """Main function to run the Simple Face Mashup Game."""
    print("🎭 Simple Face Mashup Game")
    print("=" * 35)
    print("💡 This version uses OpenCV instead of dlib")
    print("🚀 No complex dependencies required!")
    print()
    
    try:
        game = SimpleFaceMashupGame()
        game.run()
