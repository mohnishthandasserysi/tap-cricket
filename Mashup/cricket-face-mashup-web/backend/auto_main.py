...
def load_images_from_folder():
    """Auto-load all images from the uploads folder on startup"""
    global player_cache
...
print("🚀 Cricket Face Mashup API starting...")
print("📁 Auto-loading images from uploads folder...")
...
@app.post("/reload-images")
async def reload_images():
    """Manually reload images from uploads folder"""
    global player_cache
    player_cache = {}
    loaded_count = load_images_from_folder()
    return {
        "success": True,
        "message": f"Reloaded {loaded_count} images",
        "total_players": len(player_cache),
        "ready_to_play": len(player_cache) >= 3
    }
...
async def create_mashup(num_players: int = 3):
    try:
        if len(player_cache) < num_players:
            raise HTTPException(
                status_code=400, 
                detail=f"Need at least {num_players} players. Found {len(player_cache)}. Add more images to uploads folder and reload."
            )
...
async def clear_players():
    global player_cache
    player_cache = {}
    return {"success": True, "message": "All players cleared from memory (files remain in uploads folder)"}

# Mount static files to serve uploaded images
try:
    app.mount("/uploads", StaticFiles(directory="../uploads"), name="uploads")
    print("📁 Mounted uploads directory for serving images")
except Exception as e:
...
if __name__ == "__main__":
    ...
    print("📁 Place cricket player images in '../uploads' folder")
    print("🎯 Auto-loaded players:", len(player_cache))
