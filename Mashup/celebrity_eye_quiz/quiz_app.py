...
class CelebrityEyeQuiz:
    def __init__(self, images_folder="images"):
        """Initialize the Celebrity Eye Quiz game."""
    ...
    def initialize_session_state(self):
        ...
        if 'game_state' not in st.session_state:
            st.session_state.game_state = 'playing'  # 'playing', 'answered'
    ...
    def generate_question(self):
        ...
        # Crop eyes from the image
        image_path = os.path.join(self.images_folder, correct_image)
        cropped_eyes = self.face_cropper.crop_eyes_from_image(image_path)
        
        if cropped_eyes is None:
            st.error(f"Could not detect face in {correct_image}. Trying another image...")
            return self.generate_question()  # Try again with another image
    ...
    def run(self):
        ...
        if not os.path.exists(self.images_folder):
            st.error(f"Images folder '{self.images_folder}' not found! Please create it and add celebrity images.")
        ...
        if st.session_state.current_question:
            ...
            # Display cropped eye image
            st.subheader("👀 Whose eyes are these?")
            st.image(question['cropped_image'], width=300, caption="Cropped Eye Region")
...
def main():
    ...
    st.set_page_config(
        page_title="Celebrity Eye Quiz",
        page_icon="👀",
        layout="centered"
    )
    
    # Initialize and run the quiz
    quiz = CelebrityEyeQuiz()
    quiz.run()
